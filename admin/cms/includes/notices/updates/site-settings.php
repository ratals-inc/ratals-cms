...
try
{
	...
	if(!empty($all_sites_to_update))
	{
		foreach($all_sites_to_update as $site_id_to_update)
		{
			...
			{
				$update_columns = '`store_pagination` = ?, `display_stock_status` = ?, `inventory_left_instock` = ?, `call_for_availability_phone_number` = ?, `inventory_variant_builder_max` = ?, `cart_line_item_max_qty` = ?, `display_discount_code_on_cart` = ?, `allow_checkout_as_guest` = ?, `customers_save_cards` = ?, `customer_account_url` = ?, `order_confirmation_url` = ?, `free_shipping` = ?, `free_shipping_cart_minimum` = ?, `state_or_postal_code` = ?, `unit_of_measure` = ?, `unit_of_weight` = ?';
				$update_where_clause = 'WHERE `id` = ?';
				$fields_to_update = array(30, 'Yes', 5, 'Call for availability.', '3000', 9999, 'Yes', 'Yes', 'Yes', $_SESSION['install_ids'][$site_id_to_update['id']]['customer_account_page_url_id'] ?? 0, $_SESSION['install_ids'][$site_id_to_update['id']]['order_confirmation_page_url_id'] ?? 0, 'Yes', 199.0000, 'State', 'in', 'lb', $all_site_settings_to_update[$site_id_to_update['id']]['id']);
