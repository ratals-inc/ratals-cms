...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/layouts/table/ajax/delete-admin-menus.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/layouts/table/ajax/delete-admin-menus.php');
}
else
{
	//Delete Admin Menu Rows and Menu Items
	if(isset($_POST['deleteRow']) && !empty($_POST['deleteRow']) && $_POST['type'] == 'delete-admin-menus')
	{
		...
		foreach($_POST['deleteRow'] as $row_id)
		{
			...
			if($_SESSION['admin_table_name'] == "admin_menus")
			{
				$results->getDeleteRecord(__LINE__, __FILE__, 'admin_menu_items', 'WHERE `admin_menus_id` = ?', [$row_id]);
				
				$results->getDeleteRecord(__LINE__, __FILE__, 'admin_menus', 'WHERE `id` = ?', [$row_id]);
			}
			//Delete Menu Sub Items
			elseif($_SESSION['admin_table_name'] == "admin_menu_items")
			{
				//Get all subs of a menu items id
				$all_menu_items = $results->getSelectMultipleRecords(__LINE__, __FILE__, '*', 'admin_menu_items', 'WHERE `admin_menus_id` = ? AND `id` >= ? ORDER BY `id` ASC', [$get_rid, $row_id]);
				...
				if(!empty($sub_subs_to_delete_array) && !empty($get_rid))
				{
					...
					{
						$results->getDeleteRecord(__LINE__, __FILE__, 'admin_menu_items', 'WHERE `id` = ? AND `admin_menus_id` = ?', [$sub_subs_to_delete, $get_rid]);
					}
				}
