...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/column-names/tablet_media.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/column-names/tablet_media.php');
}
else
{
	if(!class_exists('tablet_media_tcn'))
	{
		class tablet_media_tcn
		{
			public function tablet_media_tcn($sql_custom_fields_rows, $sql_account_columns_active)
			{
				//Media Sliders
				if($_SESSION['admin_table_name'] == "slider_items")
				{
					...
					if(!empty($sql_custom_fields_rows["tablet_media"]))
					{
						$media_data = explode('~||~', $sql_custom_fields_rows["tablet_media"]);
		...
		$class_tablet_media_tcn = new tablet_media_tcn();
	}
	
	$class_tablet_media_tcn->tablet_media_tcn($sql_custom_fields_rows, $sql_account_columns_active);
}