...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/cf-display-as/swatch.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/cf-display-as/swatch.php');
}
else
{
	if(!class_exists('swatchTcfda'))
	{
		class swatchTcfda
		{
			public function swatchTcfda($sql_custom_fields_rows, $sql_account_columns_active, $label_names, $domain)
			{
		...
		$class_swatchTcfda = new swatchTcfda();
	}
	
	$class_swatchTcfda->swatchTcfda($sql_custom_fields_rows, $sql_account_columns_active, $label_names, $domain);
}