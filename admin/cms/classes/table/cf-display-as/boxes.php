...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/cf-display-as/boxes.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/cf-display-as/boxes.php');
}
else
{
	if(!class_exists('boxesTcfda'))
	{
		class boxesTcfda
		{
			public function boxesTcfda($sql_custom_fields_rows, $sql_account_columns_active, $label_names, $domain)
			{
		...
		$class_boxesTcfda = new boxesTcfda();
	}
	
	$class_boxesTcfda->boxesTcfda($sql_custom_fields_rows, $sql_account_columns_active, $label_names, $domain);
}