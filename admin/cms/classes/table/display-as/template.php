...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/display-as/template.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/display-as/template.php');
}
else
{
	if(!class_exists('templateTda'))
	{
		class templateTda
		{
			
			public function templateTda($sql_custom_fields_rows, $sql_account_columns_active, $dynamic_options_list, $label_names)
			{
				...
				else
				{
					...
					echo '<li class="table-cell-results '.$sql_account_columns_active["css_class"].'">'.htmlspecialchars($else_table_content ?? '').'HERE</li>';
				}
			}
		}
		
		$class_templateTda = new templateTda();
	}
	
	$class_templateTda->templateTda($sql_custom_fields_rows, $sql_account_columns_active, $dynamic_options_list, $label_names);
}