...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/display-as/dropdownId.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/display-as/dropdownId.php');
}
else
{
	if(!class_exists('dropdownIdTda'))
	{
		class dropdownIdTda
		{
			public function dropdownIdTda($sql_custom_fields_rows, $sql_account_columns_active, $dynamic_options_list, $label_names)
			{
		...
		$class_dropdownIdTda = new dropdownIdTda();
	}
	
	$class_dropdownIdTda->dropdownIdTda($sql_custom_fields_rows, $sql_account_columns_active, $dynamic_options_list, $label_names);
}