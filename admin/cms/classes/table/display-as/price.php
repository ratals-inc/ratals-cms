...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/display-as/price.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/table/display-as/price.php');
}
else
{
	if(!class_exists('priceTda'))
	{
		class priceTda
		{
			public function priceTda($sql_custom_fields_rows, $sql_account_columns_active)
			{
		...
		$class_priceTda = new priceTda();
	}
	
	$class_priceTda->priceTda($sql_custom_fields_rows, $sql_account_columns_active);
}