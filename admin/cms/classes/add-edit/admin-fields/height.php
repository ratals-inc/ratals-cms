...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/height.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/height.php');
}
else
{
	if(!class_exists('heightAeaf'))
	{
		class heightAeaf
		{
			public function heightAeaf($table_name, $admin_field, $field_value, &$errors, &$post_values)
			{
		...
		$class_heightAeaf = new heightAeaf();
	}
	
	$class_heightAeaf->heightAeaf($table_name, $admin_field, $field_value, $errors, $post_values);
}