...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/checkboxId.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/checkboxId.php');
}
else
{
	if(!class_exists('checkboxIdAeaf'))
	{
		class checkboxIdAeaf
		{
			public function checkboxIdAeaf($table_name, $admin_field, $field_value, &$errors, &$post_values)
			{
		...
		$class_checkboxIdAeaf = new checkboxIdAeaf();
	}
	
	$class_checkboxIdAeaf->checkboxIdAeaf($table_name, $admin_field, $field_value, $errors, $post_values);
}