...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/dropdownValueWithEditAsText.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/dropdownValueWithEditAsText.php');
}
else
{
	if(!class_exists('dropdownValueWithEditAsTextAeaf'))
	{
		class dropdownValueWithEditAsTextAeaf
		{
			public function dropdownValueWithEditAsTextAeaf($table_name, $admin_field, $field_value, &$errors, &$post_values, $column_data_dynamic_lists, $label_names, $current_values)
			{
				if($_SESSION['admin_type'] == 'add')
				{
					...
					if(!empty($admin_field_values))
					{
						foreach($admin_field_values as $admin_field_value)
						{
							...
							{
								$admin_field_id = $admin_field_value[$admin_field_list['dynamic_column_label']];
								$admin_field_label = $admin_field_value[$admin_field_list['dynamic_column_label']];
								
								if(!empty($admin_field_value[$admin_field_list['dynamic_column_label']]) && $field_value == $admin_field_value[$admin_field_list['dynamic_column_label']])
								{
		...
		$class_dropdownValueWithEditAsTextAeaf = new dropdownValueWithEditAsTextAeaf();
	}
	
	$class_dropdownValueWithEditAsTextAeaf->dropdownValueWithEditAsTextAeaf($table_name, $admin_field, $field_value, $errors, $post_values, $column_data_dynamic_lists, $label_names, $current_values);
}