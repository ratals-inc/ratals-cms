...
if(file_exists(INSTALLATION_ROOT.'/hooks/admin/cms/classes/add-edit/admin-fields/fileCode.php'))
{
	require_once(INSTALLATION_ROOT.'/hooks/admin/cms/classes/add-edit/admin-fields/fileCode.php');
}
else
{
	if(!class_exists('fileCodeAeaf'))
	{
		class fileCodeAeaf
		{
			public function fileCodeAeaf($table_name, $admin_field, $field_value, &$errors, &$post_values, $current_values)
			{
				...
				elseif(isset($active_template['directory_folder_name']) && !empty($active_template['directory_folder_name']) && !empty($current_values[$table_name]['filename']) && file_exists(INSTALLATION_ROOT."/sites/".$_SESSION["site_set_for_editing"]."/templates/".$active_template['directory_folder_name']."/".$current_values[$table_name]['filename']))
				{
