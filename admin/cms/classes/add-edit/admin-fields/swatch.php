...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/swatch.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/swatch.php');
}
else
{
	if(!class_exists('swatchAeaf'))
	{
		class swatchAeaf
		{
			public function swatchAeaf($table_name, $admin_field, $field_value, &$errors, &$post_values, $current_values)
			{
		...
		$class_swatchAeaf = new swatchAeaf();
	}
	
	$class_swatchAeaf->swatchAeaf($table_name, $admin_field, $field_value, $errors, $post_values, $current_values);
}