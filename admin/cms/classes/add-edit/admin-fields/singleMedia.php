...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/singleMedia.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/singleMedia.php');
}
else
{
	if(!class_exists('singleMediaAeaf'))
	{
		class singleMediaAeaf
		{
			public function singleMediaAeaf($table_name, $admin_field, $field_value, &$errors, &$post_values, $domain)
			{
				...
				if($_SESSION['admin_table_name'] == "media" && $_SESSION['admin_type'] == 'add')
				{
					echo '
				...
				else
				{
					...
					if(!empty($media))
					{
						$media_buttons = '<button type="button" class="single-media-button-'.htmlspecialchars($admin_field["column_name"] ?? '').' openSingleMediaPopup" data-click="'.htmlspecialchars($table_name.'['.$admin_field["column_name"].']' ?? '').','.$admin_field["column_name"].','.$counter.'">Change Media</button>';
					}
					else
					{
						$media_buttons = '<button type="button" class="single-media-button-'.htmlspecialchars($admin_field["column_name"] ?? '').' openSingleMediaPopup" data-click="'.htmlspecialchars($table_name.'['.$admin_field["column_name"].']' ?? '').','.$admin_field["column_name"].','.$counter.'">Add Media</button>';
					}
		...
		$class_singleMediaAeaf = new singleMediaAeaf();
	}
	
	$class_singleMediaAeaf->singleMediaAeaf($table_name, $admin_field, $field_value, $errors, $post_values, $domain);
}