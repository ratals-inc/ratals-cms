...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/metaRobots.php'))
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/classes/add-edit/admin-fields/metaRobots.php');
}
else
{
	if(!class_exists('metaRobotsAeaf'))
	{
		class metaRobotsAeaf
		{
			public function metaRobotsAeaf($table_name, $admin_field, $field_value, &$errors, &$post_values)
			{
		...
		$class_metaRobotsAeaf = new metaRobotsAeaf();
	}
	
	$class_metaRobotsAeaf->metaRobotsAeaf($table_name, $admin_field, $field_value, $errors, $post_values);
}