...
if(file_exists($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/frontend/load-page/homepage-blog.php')) 
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/hooks/admin/cms/frontend/load-page/homepage-blog.php');
}
else
{
	if(isset($template_file_type_row['assigned_type']) && $template_file_type_row['assigned_type'] == 'Pages > Homepage - Blog')
	{
		//Get pagination / blog posts per page
		if($pages_data['number_of_posts'] > 0)
		{
			$pagination_results_per_page = $pages_data['number_of_posts'];
		}
		elseif($site_settings['blog_pagination'] > 0)
		{
		...
		//Get all blog posts
		$all_blog_posts = $results->getSelectLeftJoinMultipleRecords(__LINE__, __FILE__, '*', 'posts', '`urls` ON `urls`.record_id = `posts`.`id`', 'WHERE `urls`.`site_id` = ? AND `urls`.`table_name` = ? AND `urls`.`url_status` = ? ORDER BY `created_date` DESC LIMIT '.$pagination_results_per_page, [$site_id, 'posts', 1]);
